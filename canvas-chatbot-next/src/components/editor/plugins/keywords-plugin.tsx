...
export function KeywordsPlugin(): JSX.Element | null {
  ...
  const [isKeywordNodeRegistered, setIsKeywordNodeRegistered] = useState(false)

  useEffect(() => {
    // Check if KeywordNode is available on the editor
    if (editor.hasNodes([KeywordNode])) {
      setIsKeywordNodeRegistered(true)
    } else {
      console.warn("KeywordsPlugin: KeywordNode not registered on editor")
    }
  }, [editor])
  ...
  // Always call hooks, but only use them if node is registered
  useLexicalTextEntity<KeywordNode>(
    isKeywordNodeRegistered ? getKeywordMatch : () => null,
    KeywordNode,
    $createKeywordNode_
  )
