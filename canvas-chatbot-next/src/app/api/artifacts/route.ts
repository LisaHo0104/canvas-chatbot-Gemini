...
export async function GET(request: NextRequest) {
  try {
    ...
    const artifactType = url.searchParams.get('type') // 'quiz' | 'rubric_analysis' | 'summary_note' | null
    ...
    if (artifactType && (artifactType === 'quiz' || artifactType === 'rubric_analysis' || artifactType === 'summary_note')) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!artifact_type || (artifact_type !== 'quiz' && artifact_type !== 'rubric_analysis' && artifact_type !== 'summary_note')) {
      return NextResponse.json(
        { error: 'Invalid artifact_type. Must be "quiz", "rubric_analysis", or "summary_note"' },
        { status: 400 },
      )
    }
