...
export default function FlashcardsCourseModulesPage() {
                          ...
                          <div className="flex items-center gap-2 pt-3 flex-wrap">
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                if (!itemId) return
                                console.debug('[DEBUG] Study item flashcards', {
                                  courseId,
                                  moduleId: module.id,
                                  itemId,
                                })
                                router.push(
                                  `/protected/flashcards/${courseId}/module/${module.id}?itemId=${itemId}`,
                                )
                              }}
                              disabled={!selectedItemByModule[module.id]}
                            >
                              Study flashcards for selected item
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={(e) => {
                                e.preventDefault()
                                console.debug('[DEBUG] Study module flashcards', {
                                  courseId,
                                  moduleId: module.id,
                                })
                                router.push(`/protected/flashcards/${courseId}/module/${module.id}`)
                              }}
                            >
                              Study flashcards for this module
                            </Button>
                          </div>
                        </>
                      ) : (
