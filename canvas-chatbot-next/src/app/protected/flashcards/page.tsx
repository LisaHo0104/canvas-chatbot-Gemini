...
export default function FlashcardsCoursesPage() {
  const [courses, setCourses] = useState<CanvasCourse[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [search, setSearch] = useState('')

  async function loadCourses(term?: string) {
    try {
      setLoading(true)
      setError(null)
      console.debug('[DEBUG] Fetching courses (server API)', { term: term || '' })
      const qs = new URLSearchParams()
      if (term && term.trim()) qs.set('searchTerm', term.trim())
      qs.set('enrollmentState', 'all')
      qs.set('perPage', '100')
  ...
  useEffect(() => {
    console.debug('[DEBUG] FlashcardsCoursesPage mounted')
    loadCourses(search)
  }, [])
          ...
          <>
            {courses.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                return (
                  <Link key={course.id} href={`/protected/flashcards/${course.id}`} className="block">
                    <Card className="transition-all hover:shadow-md bg-background hover:bg-accent/5 h-full">
